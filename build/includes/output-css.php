...
function las_fresh_log_css_performance_metrics($start_time, $start_memory, $start_peak_memory, $css_content, $cache_status = 'generated') {
    try {
        ...
        $metrics = array(
            ...
            'peak_memory_increase' => round(($end_peak_memory - $start_peak_memory) / 1024, 2),
...
function las_fresh_generate_admin_css_output($preview_options = null) {
    ...
    try {
        ...
        try {
            ...
            if (is_numeric($actual_value)) {
                ...
                if (in_array($property, ['opacity', 'z-index'])) {
                    ...
                    $min_value = (strpos($property, 'margin') !== false) ? null : 0;
        ...
        if ($css_value_string === 'px' && $unit === 'px' && $actual_value === 0) {
    ...
    $add_raw = function ($raw_css_block) use (&$css_output) {
        try {
            ...
            $sanitized_block = preg_replace('/\/\*.*?\*\
            $sanitized_block = preg_replace('/\s+/', ' ', $sanitized_block);
    ...
    $add_css('body.wp-admin, #wpbody-content, .wrap, #wpbody-content p, #wpbody-content li, #wpbody-content div, #wpbody-content span', 'color', 'body_text_color');
    $add_css('.wrap p, .form-table th, .form-table td, #wpbody-content input, #wpbody-content select, #wpbody-content textarea, .wp-core-ui .button', 'font-size', 'body_font_size', true, 'px');
    ...
    if ($options['body_bg_type'] === 'gradient') {
        ...
        $add_css($body_bg_selectors, 'background-color', 'body_bg_color');
    ...
    $add_css('#adminmenu .wp-submenu', 'background-color', 'admin_submenu_bg_color');

    $menu_text_sel = '#adminmenu a, #adminmenu div.wp-menu-name, #adminmenu .wp-menu-arrow, #adminmenu div.wp-menu-image::before';
    $add_css($menu_text_sel, 'color', 'admin_menu_text_color');
    ...
    $add_css($menu_text_sel, 'font-size', 'admin_menu_font_size', true, 'px');

    $submenu_text_sel = '#adminmenu .wp-submenu a, #adminmenu .wp-submenu .wp-submenu-head';
    $add_css($submenu_text_sel, 'color', 'admin_submenu_text_color');
    ...
    $add_css($submenu_text_sel, 'font-size', 'admin_submenu_font_size', true, 'px');
    ...
    $add_css($menu_item_sel, 'padding-top', 'admin_menu_padding_top_bottom', true, 'px');
    $add_css($menu_item_sel, 'padding-bottom', 'admin_menu_padding_top_bottom', true, 'px');
    $add_css($menu_item_sel, 'padding-left', 'admin_menu_padding_left_right', true, 'px');
    $add_css($menu_item_sel, 'padding-right', 'admin_menu_padding_left_right', true, 'px');
    ...
    if ($options['admin_bar_bg_type'] === 'gradient') {
        ...
        $add_css($admin_bar_selector, 'background-color', 'admin_bar_bg_color');
    ...
    $add_css($bar_text_sel, 'color', 'admin_bar_text_color');
    ...
    $add_css($bar_text_sel, 'font-size', 'admin_bar_font_size', true, 'px');
...
function las_fresh_cache_css($css_output) {
    try {
        ...
        $cache_duration = apply_filters('las_fresh_css_cache_duration', 3600);
        $cache_data = array(
            'css' => $css_output,
...
function las_fresh_get_options_hash() {
    try {
        ...
        unset($css_affecting_options['active_template']);
...
function las_fresh_generate_performance_report($operation_type = null, $days = 7) {
    try {
        ...
        if ($report['max_css_size_bytes'] > 100 * 1024) {
