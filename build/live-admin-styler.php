...
class LAS_Error_Logger {
    ...
    public function get_error_statistics() {
        $stored_errors = get_option($this->error_log_option, array());
        $cutoff_time = current_time('timestamp') - (7 * 24 * 60 * 60);
    ...
    public function cleanup_old_logs() {
        $stored_errors = get_option($this->error_log_option, array());
        $cutoff_time = current_time('timestamp') - (30 * 24 * 60 * 60);
...
class LAS_Security_Manager {
     ...
     */
    const NONCE_REFRESH_INTERVAL = 43200;
    ...
    public function check_rate_limit() {
        ...
        if (count($requests_last_minute) > self::MAX_REQUESTS_PER_MINUTE) {
            $user_limits['blocked_until'] = $current_time + 300;
        ...
        if (count($user_limits['requests']) > self::MAX_REQUESTS_PER_HOUR) {
            $user_limits['blocked_until'] = $current_time + 1800;
    ...
    public function ajax_get_security_status() {
        ...
        $recent_events = array_filter($security_log, function($entry) use ($user_id) {
            return $entry['user_id'] == $user_id && (time() - strtotime($entry['timestamp'])) < 3600;
        });
            ...
            wp_send_json_success(las_create_ajax_success_response(
                array(
                    ...
                    'security_events' => array(
                        ...
                        'events' => array_slice($recent_events, 0, 10)
                    ),
                    'nonce_status' => array(
...
function las_fresh_enqueue_plugin_scripts($dependency_config, $script_version, $preview_version) {
    ...
    wp_enqueue_script(
        ...
        array('las-fresh-error-manager-js'),
        $script_version,
        false
    );
    ...
    wp_enqueue_script(
        ...
        array('las-fresh-theme-manager-js', 'las-fresh-error-manager-js'),
        $script_version,
        false
    );
    ...
    wp_enqueue_script(
        ...
        array('las-fresh-theme-manager-js', 'las-fresh-responsive-manager-js', 'las-fresh-error-manager-js'),
        $script_version,
        false
    );
    ...
    wp_enqueue_script(
        ...
        array_merge($validated_admin_deps, $modern_ui_dependencies),
        $script_version,
        $loading_config['load_in_footer']
    );
...
function las_fresh_create_localization_data($jquery_ui_available) {
    ...
    return array(
        ...
        'live_preview' => array(
            ...
            'css_injection_method' => 'style_element',
...
function las_fresh_enqueue_plugin_styles($style_version) {
    ...
    wp_enqueue_style(
        ...
        array('las-fresh-base-reset-css'),
        $style_version
    );
    ...
    wp_enqueue_style(
        'las-fresh-admin-style-css',
        plugin_dir_url(__FILE__) . 'assets/css/admin-style.css',
        array('las-fresh-graceful-degradation-css', 'wp-color-picker', 'common', 'forms'),
        $style_version
    );
...
function las_fresh_get_script_loading_config() {
    return array(
        'load_in_footer' => true,
        'defer_non_critical' => !is_customize_preview(),
