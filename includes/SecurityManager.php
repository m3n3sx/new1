...
class LAS_SecurityManager {
    ...
    public function validateFilePath($path, $allowed_base = '') {
        ...
        // Check for absolute paths (unless specifically allowed)
    ...
    public function validateAjaxRequest($data, $action) {
        ...
        // Check rate limiting
        $rate_limit_result = $this->checkRateLimit($action, 100, 3600); // 100 requests per hour
    ...
    private function validateTemplateJSON($json) {
        ...
        // Check JSON size
        if (strlen($json) > 100000) { // 100KB limit
    ...
    public function validateSQLQuery($query, $allowed_tables = []) {
        ...
        // If allowed tables specified, check table access
