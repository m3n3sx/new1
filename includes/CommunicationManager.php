...
class LAS_CommunicationManager {
    ...
    private $config = [
        ...
        'queue_size' => 100 // maximum queued requests
    ];
    ...
    private function init_hooks() {
        ...
        // AJAX hooks will be registered dynamically
    ...
    public function registerAjaxHandler($action, $callback, $options = []) {
        ...
        $default_options = [
            ...
            'public' => false // Whether to register for non-logged-in users
        ];
        ...
        // Create wrapper function for security and retry logic
    ...
    private function handle_rest_request($request, $route, $config) {
        ...
        try {
            ...
            // Nonce validation for POST/PUT/DELETE requests
    ...
    public function rest_handle_settings($request) {
        ...
        // Get CoreEngine instance to access SettingsManager
    ...
    public function enqueue_scripts($hook_suffix) {
        // Only load on Live Admin Styler pages
        if (strpos($hook_suffix, 'live-admin-styler') === false) {
            return;
        }
        
        // Enqueue communication script
        wp_enqueue_script(
            'las-communication',
    ...
    private function sanitize_request_data($data) {
        ...
        foreach ($data as $key => $value) {
            ...
            if (is_array($value)) {
                ...
                // Basic sanitization - specific sanitization should be done by handlers
    ...
    public function cleanup_request_queue() {
        ...
        foreach ($queue as $request_id => $retry_data) {
            if (($current_time - $retry_data['created']) > 86400) { // 24 hours
                unset($queue[$request_id]);
                $cleaned++;
            }
        }
