...
class LAS_Settings_Storage {
    ...
    public function __construct() {
        // Initialize WordPress object cache group (with fallback)
        if (function_exists('wp_cache_add_global_groups')) {
            wp_cache_add_global_groups([$this->cache_group]);
        }
        
        // Hook into WordPress shutdown to process any queued batch operations (with fallback)
    ...
    public function save_settings($settings) {
        ...
        try {
            ...
            foreach ($settings as $key => $value) {
                ...
                $result = update_option($option_name, $value, false); // Don't autoload
    ...
    private function is_valid_setting_key($key) {
        // Check if key exists in defaults or follows valid pattern
        return array_key_exists($key, $this->default_settings) || 
    ...
    private function clear_related_caches($key) {
        // Clear CSS cache if color or style settings change
    ...
    public function optimize_database() {
        ...
        try {
            ...
            foreach ($options as $option) {
                // Remove autoload from LAS options to improve performance
                $wpdb->update(
                    $wpdb->options,
                    ['autoload' => 'no'],
