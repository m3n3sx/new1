...
function las_fresh_get_available_templates() {
    // Get plugin defaults to ensure templates are comprehensive or correctly inherit
    $plugin_defaults = las_fresh_get_default_options();

    return array(
        'default' => array(
            'name' => __('Domyślny (Twoje Ustawienia)', LAS_FRESH_TEXT_DOMAIN),
            'settings' => array(), // Special case: applies plugin factory defaults
        ),
        'default_wordpress' => array(
            'name' => __('Domyślny Wordpress', LAS_FRESH_TEXT_DOMAIN),
            'settings' => array_merge($plugin_defaults, array( // Start with plugin defaults and override specific WP ones
...
function las_fresh_ajax_apply_template() {
    ...
    if ( array_key_exists( $template_key, $available_templates ) ) {
        ...
        if ($template_key === 'default') {
            // 'default' template means reset to plugin's factory defaults
            $new_options = $defaults;
        } else {
            // Merge template settings onto defaults to ensure all keys are present
            $new_options = array_merge( $defaults, $template_settings );
        }
        
        // Sanitize the full set of options before sending back
