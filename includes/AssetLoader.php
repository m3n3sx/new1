...
class AssetLoader {
    ...
    private function enqueueAsset($handle, $asset) {
        ...
        if ($asset['type'] === 'css') {
            ...
            // Add critical CSS inline if specified
            ...
            // Add module attribute if specified
    ...
    private function shouldLoadAsset($handle, $asset) {
        ...
        // Always load if not conditional
    ...
    private function extractCriticalCSS($css) {
        // Extract CSS rules marked as critical
        preg_match_all('/\/\*\s*critical\s*\*\/(.*?)\/\*\s*\/critical\s*\*\//s', $css, $matches);
        
        if (!empty($matches[1])) {
            return implode("\n", $matches[1]);
        }
        
        // Fallback: extract first 2KB of CSS as critical
