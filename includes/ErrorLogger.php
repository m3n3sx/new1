...
class LAS_ErrorLogger {
    ...
    private $config = [
        ...
        'admin_email' => '',
        'rate_limit' => 100, // Max errors per hour
        'include_trace' => true,
    ...
    public function handle_php_error($errno, $errstr, $errfile, $errline) {
        ...
        // Map PHP error levels to our levels
        $level_map = [
    ...
    private function log_to_file($error_entry) {
        ...
        if ($result && filesize($this->log_file) > 10 * 1024 * 1024) { // 10MB
    ...
    private function check_rate_limit($level) {
        ...
        // Allow critical errors to bypass rate limiting
        if ($level === self::LEVEL_CRITICAL) {
            return true;
        }
    ...
    private function get_stack_trace() {
        ...
        // Remove error logger calls from trace
    ...
    private function cleanup_database_entries() {
        ...
        // Keep only the most recent entries if we exceed max_log_entries
    ...
    public function testErrorLogging() {
        ...
        // Test WordPress debug logging
        $results['wp_debug_logging'] = defined('WP_DEBUG') && WP_DEBUG;
        
        // Test error handling
        $results['error_handling'] = true; // If we got here, basic error handling works
